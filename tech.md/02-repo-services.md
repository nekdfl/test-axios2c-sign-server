...
2. `all-local` копирует `axis2.xml` в `**$(top_builddir)/axis2_repo/**`, затем `libdemo_sign.so` и `services.xml` в `**$(top_builddir)/axis2_repo/services/demo_sign/**` (при типовой сборке `python3 scripts/build.py` это каталог `**build/axis2_repo/...**`).
