...
Ниже — **пример** конфигурации: Nginx на порту **8081** проксирует на встроенный HTTP-сервер Axis2 на **127.0.0.1:8080** (тот, что поднимает `demo-sign-server` / `python3 scripts/run.py`).
...
- **8080** — порт процесса `demo-sign-server` (см. опция `-p` / переменная `PORT` в `scripts/run.py`).
...
См. также [environment.md](environment.md) (сборка и запуск) и корневой [README.md](../README.md).